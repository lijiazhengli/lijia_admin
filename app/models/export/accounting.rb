class Export::Accounting
  ...
  class << self
    def list(orders)
      ...
      total_amount_hash = OrderPaymentRecord.where(order_id: order_ids).where("timestamp is not null").group(:order_id).sum(:cost)
      order_payment_records = OrderPaymentRecord.where(order_id: order_ids).where("timestamp is not null")
      tabel_header = %w(单号 城市 状态 折扣 付款信息 产品信息 总金额 订购人姓名 订购人电话 订购人类型 推荐人 组织人 创建时间	支付时间)
      ...
      orders.each do |item|
        payed_info = ''
        product_info = ''
        payment_records = order_payment_records.select{|i| i.order_id == item.id}
        ...
        payment_records.each_with_index do |sub_item, index|
        ...
        if payment_records.present?
          result << payment_records[0].timestamp.strftime('%F %T')
        else
          result << ''
        end
